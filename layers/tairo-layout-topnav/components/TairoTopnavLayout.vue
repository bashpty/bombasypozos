...
<template>
  <div>
    <div class="dark:bg-muted-900 bg-muted-50 pb-20">
      <slot name="navigation">
        <TairoTopnavNavigation v-if="topnavEnabled" :display="props.display" position="fixed">
          <div v-if="app.tairo.topnav?.navigation?.logo?.component" class="flex h-16 w-full items-center gap-x-4">
            <NuxtLink to="/" class="flex items-center justify-center">
              <component :is="
                  resolveComponentOrNative(
                    app.tairo.topnav?.navigation.logo.component,
                  )
                " v-bind="app.tairo.topnav?.navigation.logo.props"></component>
            </NuxtLink>
            <BaseHeading v-if="app.tairo.topnav?.toolbar?.showTitle" as="h1" size="lg" weight="light"
              class="text-muted-800 hidden dark:text-white md:block">
              <slot name="title">{{ route.meta.title }}</slot>
            </BaseHeading>
            <component v-if="app.tairo.topnav?.navigation?.header?.component" :is="
                resolveComponentOrNative(
                  app.tairo.topnav?.navigation?.header?.component,
                )
              " v-bind="app.tairo.topnav?.navigation?.header?.props"></component>
    ...
    <footer class="dark:bg-muted-900 border-muted-200 dark:border-muted-700 bg-muted-50 relative border-t">
      ...
      <div class="ltablet:flex-row mx-auto flex flex-col items-center justify-between px-6 py-8 lg:flex-row" :class="[
        props.display === 'expanded-sm' && 'mx-auto w-full max-w-5xl',
        props.display === 'expanded-md' && 'mx-auto w-full max-w-6xl',
        props.display === 'expanded-lg' && 'mx-auto w-full max-w-7xl',
        props.display === 'expanded-xl' && 'mx-auto w-full',
        ...
        <NuxtLink to="/" aria-label="Go to Tairo homepage" class="ltablet:w-1/5 block w-full lg:w-1/5">
          <TairoLogoText class="text-muted-300 ltablet:mx-0 mx-auto h-4 transition-all duration-200 lg:mx-0" />
        </NuxtLink>
        <div class="ltablet:mt-0 ltablet:gap-6 mt-6 flex flex-wrap items-center justify-center gap-4 lg:mt-0 lg:gap-6">


          <NuxtLink to="https://github.com/shuriken-ui" target="_blank" rel="noopener"
          ...
          <NuxtLink to="https://cssninja.io/faq/support" target="_blank" rel="noopener"
            class="text-muted-600 hover:text-primary-500 dark:text-muted-200 dark:hover:text-primary-400 text-sm transition-colors duration-300">
            Support
          </NuxtLink>
        </div>
        <div
          ...
          <span>
            ©
            <NuxtLink to="https://cssninja.io" target="_blank" rel="noopener"
              class="text-muted-600 hover:text-primary-500 dark:text-muted-200 dark:hover:text-primary-400 text-sm transition-colors duration-300">
              Css Ninja
            </NuxtLink>
            2018-{{ year }}.
